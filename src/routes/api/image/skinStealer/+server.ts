...
export const _info: object = {
    type: "GET",
    description: "Steals a minecraft skin by modifying a few pixel from an image url, query parameters: link"
}
export const GET = async ({ url }) => {
    try {
        ...
        for (let i = 0; i < 2; i++) {
            for (let j = 0; j < 2; j++) {
              img.setPixelColor(Jimp.rgbaToInt(~~(Math.random() * 254),~~(Math.random() * 254),~~(Math.random() * 254),255),i,j);
            }
        }
